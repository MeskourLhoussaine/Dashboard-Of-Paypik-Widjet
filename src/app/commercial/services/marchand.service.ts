...
import { Marchand } from 'src/app/admin/model/merchant.model';
...
export class MarchandService {
  ...
  public deleteMarchand(id: number): Observable<Marchand> {
    return this.http.delete<Marchand>(
      environment.apiUrl + '/marchand/delete/' + id
    );
  }

  public editMarchand(marchand: Marchand): Observable<Marchand> {
    return this.http.put<Marchand>(
      environment.apiUrl + '/marchands/' + marchand.marchandId,
      marchand
    );
  }
}
